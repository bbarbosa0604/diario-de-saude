...
export async function POST(request: Request) {
  ...
  const { data: rows, error: eventsError } = await supabase.from("health_events").select("event_kind,title,detail,badge,tags").eq("user_id", userData.user.id).eq("event_date", date).order("created_at", { ascending: true }).limit(100);
  ...
  const events = (rows ?? []).map((event) => ({ tipo: event.event_kind, título: event.title, detalhe: event.detail, marcador: event.badge ?? "", tags: event.tags ?? [] }));
  // Fica na tabela profiles, fora do user_metadata/JWT (ver supabase/schema.sql).
  const { data: profileRow } = await supabase.from("profiles").select("intestinal_history").eq("user_id", userData.user.id).maybeSingle();
  const intestinalHistory = String(profileRow?.intestinal_history ?? "").slice(0, 4000);
  ...
  const prompt = `Analise os registros de saúde gastrointestinal do usuário no dia ${date}.\n\nRegistros:\n${JSON.stringify(events, null, 2)}\n\nHistórico intestinal informado pelo usuário (contexto inicial, não diagnóstico):\n${intestinalHistory || "Nenhum histórico informado."}\n\nResumos anteriores do mesmo usuário, para manter continuidade (não misture com outros usuários):\n${JSON.stringify(memories ?? [], null, 2)}\n\nEscreva uma análise objetiva em português do Brasil, com no máximo 3 parágrafos. Identifique recorrências entre refeições, suplementos, evacuações e sintomas apenas quando houver dados suficientes. Separe claramente fatos registrados de possíveis associações. Se houver apenas um dia de dados, explique que não é possível estabelecer consistência, mas ainda descreva os registros. Não invente informações, não repita frases genéricas e não inclua avisos ou disclaimers — a interface já exibirá essa orientação. Nunca afirme causa, diagnóstico, intolerância ou recomende tratamento.`;
  try {
