...
export async function POST(request: Request) {
  ...
  if (!body?.id || !body.event_date || !body.event_kind || !body.title) return NextResponse.json({ error: "Dados do evento incompletos." }, { status: 400 });
  const { data, error } = await client.from("health_events").insert({ id: body.id, user_id: userData.user.id, event_date: body.event_date, event_kind: body.event_kind, event_time: body.event_time || "00:00", title: body.title, detail: body.detail || "", badge: body.badge || null, tags: Array.isArray(body.tags) ? body.tags : [], photo_path: body.photo_path || null }).select("id,event_date,event_kind,event_time,title,detail,badge,tags,photo_path").single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
